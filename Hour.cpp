...
IMPLEMENT_DYNAMIC(Hour, CDialogEx)

Hour::Hour(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_Hour, pParent)
{
...
void Hour::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Hour, CDialogEx)
	ON_BN_CLICKED(IDOK, &Hour::OnBnClickedOk)
END_MESSAGE_MAP()
...
BOOL Hour::OnInitDialog()
{
	...
	while (!ifs.eof())//未读取完毕
	{
		...
		int n_wage = 18 * wage->m_hour;

		if (tmp.id == wage->m_id1)
		{
